#!/usr/bin/env bash
# Sanity check: greedy (temp 0) output with DFlash2, across ALL locally available
# Qwen3.8-27B target quantizations, on a given backend.
# Usage: _dflash_all_models.sh <backend-dir> [start-index]
...
for idx in "${!MODELS[@]}"; do
  ...
  m="${MODELS[$idx]}"
  MODEL="$MODELS_DIR/$m"
  LOG="$ROOT/traces/dflash_all_${BE}_${m%.gguf}.log"
  echo "=================================================================="
  echo "=== backend=$BE | target=$m ==="
  echo "=================================================================="
  ...
  grep -iE "adding speculative implementation|dflash|block_size|n_max=" "$LOG" | head -5
