# Build llama.cpp-src (b10488 + patches) -> backends/bin/rocm-72
# NOTE: full ROCm 7.2 install (C:\Program Files\AMD\ROCm\7.2) is no longer present after the
# Windows reinstall; this now builds with the ROCm 7.14 pip SDK from the project venv (.venv).
...
$BUILD = "$SRC/build-rocm72"
$TargetDir = Join-Path $Root "backends\bin\rocm-72"
...
Write-Host "Configuring llama.cpp-src (ROCm 7.14 pip SDK)..." -ForegroundColor Cyan
cmake -B $BUILD -S $SRC -G "Ninja" `
    -DCMAKE_MAKE_PROGRAM="$NINJA" `
    -DCMAKE_C_COMPILER="$ROCM_CORE\lib\llvm\bin\clang.exe" `
    -DCMAKE_CXX_COMPILER="$ROCM_CORE\lib\llvm\bin\clang++.exe" `
